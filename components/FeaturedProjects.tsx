...
// UPDATED QUERY:
// Added "&& isFeatured == true" to the filter
async function getProjects() {
  const query = `*[_type == "project" && isFeatured == true] | order(orderId asc, _createdAt desc)[0...3] {
    _id,
    title,
    year,
    orderId,
    ...
    <section className="py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project: any) => (
          <div 
            key={project._id} 
            className="group flex flex-col overflow-hidden rounded-[2rem] border border-slate-200 dark:border-white/5 bg-white dark:bg-[#0d1117] transition-all duration-500 hover:border-[#137fec]/30 hover:shadow-2xl hover:shadow-[#137fec]/5"
          >
            ...
            <div className="aspect-[16/10] bg-slate-100 dark:bg-slate-800 relative overflow-hidden m-3 rounded-[1.5rem]">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                {/* We can keep this badge or remove it since all are featured now */}
                <span className="bg-[#137fec] text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-xl shadow-lg flex items-center gap-1">
            ...
            <div className="flex flex-col p-6 pt-2 flex-1">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  {project.orderId && (
                    <span className="text-[10px] font-mono text-slate-800 dark:text-slate-600">ID_0{project.orderId}</span>
                  )}
                </div>
